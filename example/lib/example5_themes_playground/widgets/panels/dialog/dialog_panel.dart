...
class DialogPanel extends StatelessWidget {
  ...
  static final Uri _fcsFlutterFix118657 = Uri(scheme: 'https', host: 'github.com', path: 'flutter/flutter/pull/118657');
  ...
  static String _effectiveDateTimeRadiusLabel(ThemeController controller, double? radius) {
    final bool useFCS = controller.useSubThemes && controller.useFlexColorScheme;
    if (!useFCS) {
      return controller.useMaterial3 ? '28 dp' : '4 dp';
    }
    ...
    final FlexAdaptive adaptiveRadius =
        controller.adaptiveResponseRadius?.setting(controller.fakeIsWeb) ?? const FlexAdaptive.off();
    // Get the correct platform default radius.
    final double? platformRadius =
        adaptiveRadius.adapt(controller.platform) ? controller.defaultRadiusAdaptive : controller.defaultRadius;

    // Use adaptive dialog radius?
    final FlexAdaptive adaptiveDialogRadius =
        controller.adaptiveResponseDialogRadius?.setting(controller.fakeIsWeb) ?? const FlexAdaptive.off();
    // Get the effective used adaptive dialog default radius.
    final double? platformDialogRadius =
        adaptiveDialogRadius == const FlexAdaptive.off() && controller.dialogBorderRadius == null
            ? null
  ...
  static String _datePickerHeaderBackground(ThemeController controller, bool isLight) {
    final bool useFCS = controller.useSubThemes && controller.useFlexColorScheme;
    if (!useFCS ||
    ...
    if (isLight) {
      return SchemeColor.values[controller.dialogBackgroundLightSchemeColor!.index].name;
    } else {
      return SchemeColor.values[controller.dialogBackgroundDarkSchemeColor!.index].name;
    }
  }

  // Complex logic for the date picker default Header foreground color.
  static String _datePickerHeaderForeground(ThemeController controller, bool isLight) {
    final bool useFCS = controller.useSubThemes && controller.useFlexColorScheme;
    if (!useFCS ||
        (controller.dialogBackgroundLightSchemeColor == null && isLight) ||
        (controller.dialogBackgroundDarkSchemeColor == null && !isLight) ||
        (!controller.useMaterial3 && controller.datePickerHeaderBackgroundSchemeColor == null)) {
      return controller.useMaterial3
          ? 'onSurfaceVariant'
          : isLight
          ? 'onPrimary'
    ...
    if (isLight && controller.datePickerHeaderForegroundSchemeColor == null) {
      return SchemeColor
          ...
          .name;
    } else if (!isLight && controller.datePickerHeaderForegroundSchemeColor == null) {
      return SchemeColor
          .values[FlexSubThemes.onSchemeColor(
    ...
    return Column(
      children: <Widget>[
        ...
        const ListTileReveal(
          ...
          subtitleReveal: Text(
            'In Flutter Material-2 the default dialog '
            ...
            text: TextSpan(
              children: <TextSpan>[
                TextSpan(
                  style: spanTextStyle,
                  text:
                      'In Flutter 3.22 to 3.24 in M3 mode, the Dialog and '
                ...
                LinkTextSpan(style: linkStyle, uri: _fcsFlutterIssue148849, text: 'issue #148849'),
                TextSpan(style: spanTextStyle, text: '. This is is fixed in Flutter 3.27.\n'),
              ],
            ),
          ),
        ),
        ...
        SliderListTileReveal(
          ...
          valueHeading: 'RADIUS',
          valueUnitLabel: ' dp',
          valueDefaultLabel: _effectiveDateTimeRadiusLabel(controller, controller.timePickerDialogBorderRadius),
        ),
        SliderListTileReveal(
          enabled: enableControl,
            ...
            text: TextSpan(
              children: <TextSpan>[
                TextSpan(
                  style: spanTextStyle,
                  text:
                      'In Flutter 3.13 and earlier the clock dial background '
                      'used the wrong default background color in Material-3 '
                      'mode. For more info see ',
                ),
                LinkTextSpan(style: linkStyle, uri: _fcsFlutterFix118657, text: 'issue #118657'),
                TextSpan(
                  style: spanTextStyle,
                  text:
                      '. In Flutter 3.16 and later the issue has been fixed. '
        ...
        const ListTileReveal(
          ...
          subtitleReveal: Text(
            'Flutter 3.7 does not support any Material-3 styling '
            ...
            text: TextSpan(
              children: <TextSpan>[
                TextSpan(
                  style: spanTextStyle,
                  text:
                      'In Flutter 3.10 in Material-3 mode, the DatePicker '
                ...
                LinkTextSpan(style: linkStyle, uri: _fcsFlutterIssue126597, text: 'issue #126597'),
                TextSpan(
                  style: spanTextStyle,
                  text:
                      '. Both the Divider spacing and color styling have '
                ...
                LinkTextSpan(style: linkStyle, uri: _fcsFlutterIssue126617, text: 'issue #126617'),
                TextSpan(
                  style: spanTextStyle,
                  text:
                      '. This issue has a feature in Flutter 3.13 to enable '
                      'using another decorator, but the solution is partially '
                      'flawed, see ',
                ),
                LinkTextSpan(style: linkStyle, uri: _fcsFlutterPull128950, text: 'PR comment #128950'),
                TextSpan(style: spanTextStyle, text: '. This topic is further discussed in '),
                LinkTextSpan(style: linkStyle, uri: _fcsFlutterIssue131666, text: 'issue #126617'),
                TextSpan(
                  style: spanTextStyle,
                  text:
