...
class _ThemeSimulatorPanelState extends State<ThemeSimulatorPanel> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    tabController = TabController(initialIndex: currentPage, length: 5, vsync: this);
        ...
        children: <Widget>[
          ...
          Row(
            ...
            children: <Widget>[
                  ...
                  child: ListTileReveal(
                    ...
                    leading: IconButton(
                      icon: AnimatedRotation(
                        ...
                        child: Icon(Icons.screen_rotation_outlined, color: theme.colorScheme.primary, size: 30),
                      ),
                      onPressed: () {
                        setState(() {
              ...
              const Padding(padding: EdgeInsets.only(top: 14.0), child: Text('Zoom')),
              Expanded(
                flex: 1,
                child: Slider(
                  min: 400,
          ...
          Card(
            ...
            child: ConstrainedBox(
              constraints: BoxConstraints(maxHeight: widget.controller.simulatorViewZoom),
              child: Padding(
...
class SimulatorFrame extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DeviceFrame(
      ...
      screen: Builder(
        builder:
            (BuildContext deviceContext) => MaterialApp(
...
// and icons.
class SimDevice {
  const SimDevice({String? name, this.icon = Icons.phone_iphone, this.size = 24, required this.info}) : _name = name;
...
class SimulatorPopupMenu extends StatelessWidget {
  const SimulatorPopupMenu({super.key, required this.index, this.onChanged, this.child});
  final int index;
  ...
  static List<SimDevice> devices = <SimDevice>[
    //
    // iPhones here
    SimDevice(info: Devices.ios.iPhone13ProMax, size: 30),
    SimDevice(info: Devices.ios.iPhone13, size: 26),
    SimDevice(info: Devices.ios.iPhone13Mini, size: 22),
    SimDevice(info: Devices.ios.iPhone12ProMax, size: 30),
    SimDevice(info: Devices.ios.iPhone12, size: 26),
    SimDevice(info: Devices.ios.iPhone12Mini, size: 22),
    SimDevice(info: Devices.ios.iPhoneSE, size: 20),
    SimDevice(info: Devices.ios.iPhone14Pro, size: 26),
    //
    // iPads here
    SimDevice(info: Devices.ios.iPad, icon: Icons.tablet_mac_outlined, size: 28),
    SimDevice(info: Devices.ios.iPadAir4, icon: Icons.tablet_mac_outlined, size: 30),
    SimDevice(info: Devices.ios.iPadPro11Inches, icon: Icons.tablet_mac_outlined, size: 32),
    SimDevice(info: Devices.ios.iPad12InchesGen4, icon: Icons.tablet_mac_outlined, size: 36),
    //
    // Android brands here
    SimDevice(info: Devices.android.samsungGalaxyNote20Ultra, icon: Icons.smartphone_outlined, size: 30),
    SimDevice(info: Devices.android.samsungGalaxyNote20, icon: Icons.smartphone_outlined, size: 28),
    SimDevice(info: Devices.android.samsungGalaxyS20, icon: Icons.smartphone_outlined, size: 26),
    SimDevice(info: Devices.android.samsungGalaxyA50, icon: Icons.smartphone_outlined, size: 28),
    SimDevice(info: Devices.android.pixel4, icon: Icons.smartphone_outlined, size: 26),
    SimDevice(info: Devices.android.onePlus8Pro, icon: Icons.smartphone_outlined, size: 26),
    SimDevice(info: Devices.android.sonyXperia1II, icon: Icons.smartphone_outlined, size: 30),
    //
    // Android generic phones.
    SimDevice(info: Devices.android.bigPhone, icon: Icons.phone_android_outlined, name: 'Big Android phone', size: 30),
    SimDevice(
            ...
            for (int i = 0; i < devices.length; i++)
              PopupMenuItem<int>(
                ...
                child: ListTile(
                  ...
                  leading:
                      index == i
                          ? Icon(devices[i].icon, size: devices[i].size, color: iconColor)
                          : Icon(devices[i].icon, size: devices[i].size, color: unselectedIconColor),
