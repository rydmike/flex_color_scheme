...
class RandomColor {
  ...
  MaterialColor randomMaterialColor({
    ...
    final MaterialColor finishedColor = MaterialColor(baseColor.value32bit, <int, Color>{
  ...
  List<Color> randomColors({
    ...
    for (int i = 0; i < count; i++) {
      colors.add(randomColor(colorHue: colorHue, colorSaturation: colorSaturation, colorBrightness: colorBrightness));
    }
    return colors;
  }
...
class ColorBrightness {
  const ColorBrightness.custom(Range brightnessRange) : _brightness = brightnessRange, type = -1;
  ...
  static const ColorBrightness veryLight = ColorBrightness._(
    Range((maxBrightness + minBrightness) ~/ 2, maxBrightness + (minBrightness ~/ 2)),
    0,
  );
...
@immutable
class ColorHue {
  const ColorHue.custom(Range hueRange) : _hue = hueRange, type = -1;
  ...
  static const List<ColorHue> values = <ColorHue>[red, orange, yellow, green, blue, purple, pink, random];
  ...
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is ColorHue && runtimeType == other.runtimeType && _hue == other._hue && type == other.type;
...
class ColorSaturation {
  const ColorSaturation.custom(Range saturationRange) : _saturation = saturationRange, type = -1;
...
class Range {
  ...
  const Range.staticValue(int value) : start = value, end = value;
  const Range.zero() : start = 0, end = 0;
