...
- Documentation: default and compatibility claims must match implementation. Flag new unsupported Flutter imports and missing regression coverage where they affect the change. Flag `[Symbol]` in function or factory **parameter-level** `///` comments (they do not resolve; body comments should use `## [paramName]` catalogs with `[Symbol]`, while parameter blurbs use backticks).
