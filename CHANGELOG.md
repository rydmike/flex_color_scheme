...
**Themes Playground**
- Themes Playground got surface blend mode defaults/resets that are mobile friendly. No API change involved, API default same as before. Previously used Playground default values were intended for desktop/tablet designs, where controls and text is placed on containers with a lower surface blend, like the Cards used in the Themes Playground app itself. While one can make a responsive app that does so nicely for mobile, tablet and desktop sizes too, most mobile firs apps are not designed to do this. Starting with Playground defaults that produces a nice theme for more typical mobile designs should help new Flutter users get started with it quicker.
...
For the actual list of changes that are in 6.0.0.dev.1, please refer to release 6.0.0 above.
The change list is identical and were all included in 6.0.0.
