...
void main() {
  ...
  group('FSTC1: WITH FlexSubThemesData ', () {
    ...
    const FlexSubThemesData m3 = FlexSubThemesData(
      ...
      sliderValueTinted: false,
      //
      inputDecoratorFocusedHasBorder: false, //<- Diff,
      inputDecoratorUnfocusedHasBorder: true,
      fabUseShape: false,
      fabAlwaysCircular: false,
      searchUseGlobalShape: false,
    ...
    test('FSTC1.01a: GIVEN the same FlexSubThemeConfig object '
        'EXPECT them to be equal', () {
      expect(m1, equals(m1));
    });
    test('FSTC1.01b: GIVEN the same FlexSubThemeConfig object '
        'EXPECT them to have identity', () {
      expect(identical(m1, m1), true);
    });
    test('FSTC1.01c: GIVEN two equal FlexSubThemeConfig objects '
        'EXPECT them to be equal', () {
      expect(m1, equals(m2));
    });
    test('FSTC1.01d: GIVEN two equal FlexSubThemeConfig objects '
        'EXPECT them to have identity', () {
      expect(identical(m1, m2), true);
    });
    test('FSTC1.01e: GIVEN two equal FlexSubThemeConfig objects '
        'EXPECT them to have equality with operator', () {
      expect(m1 == m2, true);
    });
    test('FSTC1.02a: GIVEN none equal FlexSubThemeConfig objects '
        'EXPECT them to be unequal', () {
      expect(m1, isNot(m3));
    });
    test('FSTC1.02b: GIVEN none equal FlexSubThemeConfig objects '
        'EXPECT them to be unequal with operator', () {
      expect(m1 != m3, true);
    });
    test('FSTC1.02c: GIVEN none equal FlexSubThemeConfig objects '
        'EXPECT them to be equal after copy with change to make equal', () {
      expect(
        m1,
        equals(
          m3.copyWith(inputDecoratorFocusedHasBorder: true),
        ),
      );
    });
    test('FSTC1.03a: GIVEN a FlexSubThemeConfig object EXPECT it to be '
    ...
    test('FSTC1.03b: GIVEN a FlexSubThemeConfig object EXPECT it to be '
    ...
    const FlexSubThemesData m5 = FlexSubThemesData(
      ...
      //
      cardBackgroundSchemeColor: SchemeColor.surfaceContainerHighest,
      cardBorderSchemeColor: SchemeColor.error,
      cardBorderWidth: 1.5,
      cardRadius: 20,
      cardElevation: 2,
      //
      ...
      tooltipOpacity: 0.67,
      //
      progressIndicatorBaseSchemeColor: SchemeColor.secondary,
      progressIndicatorLinearTrackSchemeColor: SchemeColor.onSecondaryContainer,
      progressIndicatorLinearMinHeight: 6,
      progressIndicatorCircularTrackSchemeColor: SchemeColor.tertiary,
      progressIndicatorRefreshBackgroundSchemeColor: SchemeColor.surfaceContainerLow,
      progressIndicatorLinearRadius: 4,
      progressIndicatorStopIndicatorSchemeColor: SchemeColor.primaryFixed,
      progressIndicatorStrokeAlign: 1,
      progressIndicatorStrokeCap: StrokeCap.round,
      progressIndicatorConstraints: BoxConstraints(minWidth: 45.0, minHeight: 45.0),
      progressIndicatorTrackGap: 7,
      progressIndicatorCircularTrackPadding: EdgeInsets.all(3.0),
    ...
    test('FSTC1.03c: GIVEN a FULL FlexSubThemeConfig object EXPECT it to be '
        'equal to an unequal object when made equal with copyWith.', () {
      expect(
        m5,
        equals(
          m4.copyWith(
            ...
            adaptiveAppBarScrollUnderOff: const FlexAdaptive.excludeAndroidFuchsia(),
            ...
            progressIndicatorStrokeAlign: 1,
            progressIndicatorStrokeCap: StrokeCap.round,
            progressIndicatorConstraints: const BoxConstraints(minWidth: 45.0, minHeight: 45.0),
            progressIndicatorTrackGap: 7,
            progressIndicatorCircularTrackPadding: const EdgeInsets.all(3.0),
            progressIndicatorYear2023: false,
            //
            useInputDecoratorThemeInDialogs: true,
            //
    ...
    //**************************************************************************
    test('FSTC1.06: Test toString implemented via debugFillProperties '
    ...
    test('FSTC1.08c: Test hashCode copyWith has same exact value.', () {
      expect(m1.hashCode, equals(m1.copyWith(interactionEffects: false).hashCode));
    });
  });
}
