...
void main() {
  ...
  group('Test ElevationOverlay.applyOverlay', () {
    ...
    Widget testApp(ThemeMode themeMode, ThemeData lightTheme, ThemeData darkTheme) {
      return MaterialApp(
        theme: lightTheme,
        darkTheme: darkTheme,
        themeMode: themeMode,
    ...
    testWidgets('A) Expect no overlay in light ThemeData.from() with default '
      ...
      final Color container = Theme.of(savedContext).colorScheme.surfaceContainer;

      // EXPECT: no elevation overlay color for none elevated surface.
      expect(surface, equals(ElevationOverlay.applyOverlay(savedContext, surface, 0)));
      // EXPECT: no elevation overlay color for elevated surface.
      expect(surface, equals(ElevationOverlay.applyOverlay(savedContext, surface, 1)));

      // EXPECT: no elevation overlay color for none elevated container.
      expect(container, equals(ElevationOverlay.applyOverlay(savedContext, container, 0)));
      // EXPECT: no elevation overlay color for elevated container.
      expect(container, equals(ElevationOverlay.applyOverlay(savedContext, container, 1)));
    });

    testWidgets('B) Expect no overlay in light ThemeData.from() with container '
        'and surface colors using same none default color.', (WidgetTester tester) async {
    ...
    testWidgets('C) Expect no overlay in light ThemeData.from() with container '
        'and surface colors using different none default color.', (WidgetTester tester) async {
    ...
    testWidgets('D) Expect overlay in dark ThemeData.from() with default container '
      ...
      expect(surface, isNot(ElevationOverlay.applyOverlay(savedContext, surface, 1)));

      // EXPECT: no elevation overlay color for none elevated container.
      expect(container, equals(ElevationOverlay.applyOverlay(savedContext, container, 0)));
      // EXPECT: elevation overlay color for elevated container.
      expect(container, isNot(ElevationOverlay.applyOverlay(savedContext, container, 1)));
    });

    testWidgets('E) Expect overlay in dark ThemeData.from() with container and '
        'surface colors using same none default color, when elevation > 0.', (WidgetTester tester) async {
      // None default surface and container colors, but equal.
      final ThemeData light = lightTheme(0x10);
      final ThemeData dark = darkTheme(0x10);
    ...
    testWidgets('F1) Expect NO overlay in dark ThemeData.from() on container and '
    ...
    testWidgets('F2) Expect *NO* overlay in dark ThemeData.from() on surfaceContainer, '
        'when different from surface color and using separate and none default '
        'color, when elevation > 0. '
        'THIS IS REVERSE EXPECTATION: '
        'WE WOULD EXPECT ELEVATION ON THIS AND THIS TEST TO FAIL!', (WidgetTester tester) async {
      // None default and not equal surface and container colors.
      final ThemeData light = lightTheme(0x10);
      final ThemeData dark = darkTheme(0x10);
    ...
    testWidgets('G1) Expect **NO** overlay in dark ThemeData() on container and '
        'surface colors when using default ThemeData() factory with '
        'applyElevationOverlayColor = true and elevation = 0.', (WidgetTester tester) async {
      // Light and dark theme data made using
      // ThemeData(applyElevationOverlayColor: true) factory.
      final ThemeData light = ThemeData(applyElevationOverlayColor: true, useMaterial3: false);
    ...
    testWidgets('G2) Expect *NO* overlay in dark ThemeData() on container, when '
      ...
      // ThemeData(applyElevationOverlayColor: true) factory.
      final ThemeData light = ThemeData(applyElevationOverlayColor: true, useMaterial3: false);
      ThemeData dark = ThemeData(brightness: Brightness.dark, applyElevationOverlayColor: true, useMaterial3: false);
      dark = dark.copyWith(colorScheme: dark.colorScheme.copyWith(surfaceContainer: Colors.green));
